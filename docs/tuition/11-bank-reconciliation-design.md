...
thủ công. Nếu một giao dịch gộp nhiều QR, giao diện hiển thị các nhóm batch có
tổng tiền khớp để người dùng chọn. Backend vẫn xác thực token, phương thức, tài
khoản, trạng thái batch và toàn bộ allocation.

Chỉ cho xác nhận khi tìm được đúng payment batch đang `PENDING` và số tiền khớp.
Với nhóm, tất cả batch phải cùng tài khoản, là `BANK_TRANSFER`, đang `PENDING`, và
tổng tiền phải bằng số tiền giao dịch. Xác nhận nhóm là một transaction nguyên tử:
