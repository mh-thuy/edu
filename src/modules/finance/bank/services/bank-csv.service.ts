...
export async function confirmBankTransaction(transactionId: string, tuitionFeeId: string, actorId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    const payment = await tx.tuitionPayment.create({ data: { paymentNo: `PAY-BANK-${Date.now()}`, tuitionFeeId: fee.id, studentId: fee.studentId, paymentDate: transaction.transactionDate, amount: fee.finalAmount, paymentMethod: "BANK_TRANSFER", paymentStatus: "SUCCESS", bankAccountId: transaction.bankAccountId, bankTransactionNo: transaction.bankTransactionNo, transactionReference: transaction.transactionHash, paymentContent: transaction.description, receivedBy: actorId, confirmedBy: actorId, confirmedAt: new Date(), createdBy: actorId, updatedBy: actorId } });
    const receipt = await tx.tuitionReceipt.create({ data: { receiptNo: `REC-BANK-${Date.now()}`, paymentId: payment.id, issuedBy: actorId, receiverName: fee.studentId, amount: fee.finalAmount } });
