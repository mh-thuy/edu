...
import { Prisma, BankImportStatus, BankReconciliationStatus, BankMatchMethod } from "@prisma/client";
...
function parseMoney(value: string): Prisma.Decimal {
  const normalized = value.replace(/VND/gi, "").replace(/\s/g, "").replace(/,/g, "");
...
export async function importBankCsv(args: { buffer: Buffer; fileName: string; fileUrl: string; bankAccountId: string; actorId: string }) {
  ...
  const existing = await prisma.bankStatementImport.findUnique({ where: { bankAccountId_fileHash: { bankAccountId: args.bankAccountId, fileHash } } });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const row of rows) {
      ...
      const batches = isCredit ? await tx.paymentBatch.findMany({ where: { status: "PENDING", totalAmount: row.amount }, include: { allocations: { include: { tuitionFee: true } }, student: true } }) : [];
      const batch = batches.find((candidate) => description.includes(normalize(candidate.batchNo)));
      if (batch) {
        matchedRows += 1;
        await tx.bankStatementTransaction.create({ data: { statementImportId: statement.id, bankAccountId: args.bankAccountId, rowNo: row.rowNo, bankTransactionNo: row.transactionNo, transactionDate: row.transactionDate, description: row.description, creditAmount: row.amount, debitAmount: 0, balanceAmount: row.balance, transactionHash, reconciliationStatus: BankReconciliationStatus.AUTO_MATCHED, matchScore: 100, matchMethod: BankMatchMethod.PAYMENT_REFERENCE, matchedStudentId: batch.studentId, matchedTuitionFeeId: batch.allocations[0]?.tuitionFeeId, paymentBatchId: batch.id } });
        continue;
      }
      const candidates = fees.map((fee) => {
        ...
        return { fee, score, method: codeMatch ? BankMatchMethod.STUDENT_CODE : nameMatch ? BankMatchMethod.STUDENT_NAME : BankMatchMethod.AMOUNT };
      }).sort((a, b) => b.score - a.score);
      const selected = candidates[0];
      const status = !isCredit ? BankReconciliationStatus.IGNORED : selected && selected.score >= 80 ? (candidates.length === 1 ? BankReconciliationStatus.AUTO_MATCHED : BankReconciliationStatus.AMBIGUOUS) : BankReconciliationStatus.UNMATCHED;
      const selectedFee = selected?.fee;
      if (status === BankReconciliationStatus.AUTO_MATCHED) matchedRows += 1; else if (isCredit) unmatchedRows += 1;
...
export async function confirmBankTransaction(transactionId: string, tuitionFeeId: string, actorId: string) {
  return prisma.$transaction(async (tx) => {
    const transaction = await tx.bankStatementTransaction.findUnique({ where: { id: transactionId }, include: { candidates: true } });
    ...
    if (!transaction.creditAmount.equals(fee.finalAmount)) throw new ConflictError("Số tiền sao kê không khớp số tiền học phí");
    const uniqueToken = `${Date.now()}-${Math.floor(Math.random() * 1000000)}`;
    const payment = await tx.tuitionPayment.create({ data: { paymentNo: `PAY-BANK-${uniqueToken}`, tuitionFeeId: fee.id, studentId: fee.studentId, paymentDate: transaction.transactionDate, amount: fee.finalAmount, paymentMethod: "BANK_TRANSFER", paymentStatus: "SUCCESS", bankAccountId: transaction.bankAccountId, bankTransactionNo: transaction.bankTransactionNo, transactionReference: transaction.transactionHash, paymentContent: transaction.description, receivedBy: actorId, confirmedBy: actorId, confirmedAt: new Date(), createdBy: actorId, updatedBy: actorId } });
    const receipt = await tx.tuitionReceipt.create({ data: { receiptNo: `REC-BANK-${uniqueToken}`, paymentId: payment.id, issuedBy: actorId, receiverName: fee.studentId, amount: fee.finalAmount } });
    await tx.tuitionFee.update({ where: { id: fee.id }, data: { status: "PAID", version: { increment: 1 }, updatedBy: actorId } });
    const updated = await tx.bankStatementTransaction.update({ where: { id: transactionId }, data: { matchedStudentId: fee.studentId, matchedTuitionFeeId: fee.id, paymentId: payment.id, reconciliationStatus: "CONFIRMED", confirmedBy: actorId, confirmedAt: new Date(), matchMethod: "MANUAL" }, include: { statementImport: true } });
    await tx.tuitionAuditLog.create({ data: { entityType: "BANK_STATEMENT_TRANSACTION", entityId: transactionId, action: "CONFIRM", dataAfter: { paymentId: payment.id, tuitionFeeId: fee.id }, performedBy: actorId } });
    return { transaction: updated, payment, receipt };
  });
}

export async function confirmBankBatchTransaction(transactionId: string, batchId: string, actorId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (!transaction.creditAmount.equals(batch.totalAmount)) throw new ConflictError("Số tiền sao kê không khớp tổng batch thanh toán");
    const result = await completePaymentBatch(tx, batchId, actorId, { paymentDate: transaction.transactionDate, bankAccountId: transaction.bankAccountId, bankTransactionNo: transaction.bankTransactionNo || undefined, transactionReference: transaction.transactionHash || undefined, paymentContent: transaction.description || undefined });
    const updated = await tx.bankStatementTransaction.update({ where: { id: transactionId }, data: { paymentBatchId: batchId, matchedStudentId: batch.studentId, reconciliationStatus: "CONFIRMED", confirmedBy: actorId, confirmedAt: new Date(), matchMethod: "PAYMENT_REFERENCE" }, include: { statementImport: true } });
