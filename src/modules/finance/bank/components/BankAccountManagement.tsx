...
export function BankAccountManagement() {
  ...
  return <Stack spacing={{ xs: 2, md: 3 }}><Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, border: "1px solid", borderColor: "divider", borderRadius: 3 }}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }} gap={2}><BoxTitle /><Button variant="contained" startIcon={<AddIcon />} onClick={startCreate}>Thêm tài khoản</Button></Stack></Paper>{error && <Alert severity="error" action={<Button color="inherit" size="small" startIcon={<RefreshOutlinedIcon />} onClick={() => void load()} disabled={loading}>Thử lại</Button>}>{error}</Alert>}<Paper sx={{ p: { xs: 2, md: 2.5 } }}><Stack direction={{ xs: "column", sm: "row" }} alignItems={{ sm: "center" }} justifyContent="space-between"><Typography variant="subtitle2">Tài khoản sử dụng cho thu học phí</Typography><FormControlLabel control={<Switch checked={showInactive} onChange={(event) => setShowInactive(event.target.checked)} />} label="Hiển thị tài khoản đã ngưng sử dụng" /></Stack></Paper><Paper sx={{ overflow: "hidden" }}><Typography variant="caption" color="text.secondary" sx={{ display: { xs: "block", md: "none" }, px: 2, pt: 1.5, pb: 1 }}>Vuốt ngang để xem đầy đủ thông tin và thao tác.</Typography><Box sx={{ overflowX: "auto" }}><Table size="small" sx={{ width: "100%", minWidth: { xs: 980, md: 0 } }}><TableHead><TableRow><TableCell sx={{ minWidth: { xs: 240, md: 0 } }}>Ngân hàng</TableCell><TableCell sx={{ minWidth: { xs: 160, md: 0 } }}>Số tài khoản</TableCell><TableCell sx={{ minWidth: { xs: 180, md: 0 } }}>Chủ tài khoản</TableCell><TableCell sx={{ minWidth: { xs: 140, md: 0 } }}>Chi nhánh</TableCell><TableCell sx={{ minWidth: { xs: 90, md: 0 } }}>Tiền tệ</TableCell><TableCell sx={{ minWidth: { xs: 120, md: 0 } }}>Trạng thái</TableCell><TableCell sx={{ minWidth: { xs: 230, md: 0 } }} /></TableRow></TableHead><TableBody>{!loading && items.map((item) => <TableRow key={item.id}><TableCell sx={{ minWidth: { xs: 240, md: 0 } }}>{item.bankName}<br /><Typography variant="caption" color="text.secondary">{item.bankCode}</Typography></TableCell><TableCell sx={{ minWidth: { xs: 160, md: 0 } }}>{item.accountNo}</TableCell><TableCell sx={{ minWidth: { xs: 180, md: 0 } }}>{item.accountName}</TableCell><TableCell sx={{ minWidth: { xs: 140, md: 0 } }}>{item.branchName || "-"}</TableCell><TableCell sx={{ minWidth: { xs: 90, md: 0 } }}>{item.currencyCode}</TableCell><TableCell sx={{ minWidth: { xs: 120, md: 0 } }}><Chip size="small" color={item.isActive ? "success" : "default"} label={item.isActive ? "Đang dùng" : "Đã ngưng"} /></TableCell><TableCell sx={{ minWidth: { xs: 230, md: 0 } }}><Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap" justifyContent="flex-end"><Button size="small" variant="outlined" startIcon={<EditOutlinedIcon />} onClick={() => startEdit(item)}>Sửa</Button><Button size="small" variant="outlined" color={item.isActive ? "warning" : "success"} disabled={togglingId === item.id} onClick={() => void toggle(item)}>{togglingId === item.id ? "Đang cập nhật..." : item.isActive ? "Ngưng dùng" : "Kích hoạt"}</Button></Stack></TableCell></TableRow>)}{!loading && !items.length && <TableRow><TableCell colSpan={7}><Typography sx={{ p: 4 }} textAlign="center" color="text.secondary">Chưa có tài khoản ngân hàng</Typography></TableCell></TableRow>}{loading && <TableRow><TableCell colSpan={7}><Typography sx={{ p: 4 }} textAlign="center">Đang tải...</Typography></TableCell></TableRow>}</TableBody></Table></Box></Paper><AccountDialog open={open} editing={editing} saving={saving} error={dialogError} onClose={() => { setDialogError(""); setOpen(false); }} onSave={save} />{Snackbar}</Stack>;
}
