...
export function TuitionPaymentWorkspace() {
  ...
  return <Stack spacing={3} sx={{ width: "100%" }}>
    ...
    {step === 3 && <Card><CardContent><Stack spacing={2} alignItems="center" textAlign="center"><Chip color={pendingBatch ? "warning" : "success"} label={pendingBatch ? "Đang chờ đối soát" : "Thanh toán thành công"} /><Typography variant="h6">{pendingBatch ? `Mã batch: ${pendingBatch.batchNo}` : "Đã ghi nhận toàn bộ khoản đã chọn"}</Typography>{pendingBatch && <><Alert severity="info">Nội dung chuyển khoản cần có mã batch <strong>{pendingBatch.batchNo}</strong>.</Alert><Box component="img" src={pendingBatch.qrUrl} alt="QR chuyển khoản" sx={{ width: 260, height: 260 }} /><Stack direction={{ xs: "column", sm: "row" }} spacing={1}><Button variant="outlined" href={`/api/payment-batches/${pendingBatch.id}/notice/pdf`}>Tải thông báo PDF</Button><Button variant="contained" href={`/api/payment-batches/${pendingBatch.id}/notice/pdf?inline=1`} target="_blank" rel="noopener noreferrer">Mở để in</Button></Stack></>}{receiptId && <Button variant="outlined" onClick={() => window.open(`/api/payment-batch-receipts/${receiptId}/pdf`, "_blank", "noopener,noreferrer")}>Xuất biên lai tổng</Button>}<Button onClick={reset}>Thu học phí cho học sinh khác</Button></Stack></CardContent></Card>}
