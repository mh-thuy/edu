...
export async function generatePaymentBatchNoticePdf(batchId: string) {
  ...
  if (batch.status !== PaymentBatchStatus.PENDING) {
    throw new ConflictError("Chỉ có thể xuất thông báo cho batch đang chờ đối soát");
  }
  ...
  const draw = (text: string, x: number, y: number, size = 11, textColor = color) =>
  ...
  draw(`Mã batch: ${batch.batchNo}`, 55, 708);
  ...
  draw(`Phương thức: ${batch.paymentMethod === "BANK_TRANSFER" ? "Chuyển khoản / VietQR" : batch.paymentMethod}`, 75, y - 75);
  draw(`Nội dung chuyển khoản: PB ${batch.batchNo}`, 75, y - 97);
  ...
  if (qrUrl) {
    ...
    if (qrResponse.ok) {
      const qr = await pdf.embedPng(Buffer.from(await qrResponse.arrayBuffer()));
      page.drawImage(qr, { x: 395, y: bankY - 155, width: 125, height: 125 });
    }
  }
  draw("Vui lòng ghi đúng mã batch khi chuyển khoản.", 75, 90, 9, muted);
