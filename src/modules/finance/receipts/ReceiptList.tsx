...
type Receipt = { id: string; receiptNo: string; issuedAt: string; amount: number; status: "ACTIVE" | "CANCELLED"; payment: { paymentNo: string; paymentMethod: string; tuitionFee: { feeNo: string; student: { code: string; fullName: string }; class: { name: string }; items: Array<{ classSubject: { subject: { name: string } } | null }> } } };
    ...
    <Stack spacing={2.5}>
      ...
      <Paper sx={{ overflow: "hidden" }}>
        <Table sx={{ minWidth: 820 }}>
          <TableHead><TableRow><TableCell>Số biên lai</TableCell><TableCell>Học viên</TableCell><TableCell>Khoản thu</TableCell><TableCell>Môn đã đăng ký</TableCell><TableCell>Ngày phát hành</TableCell><TableCell>Trạng thái</TableCell><TableCell align="right">Số tiền</TableCell><TableCell /></TableRow></TableHead>
          <TableBody>
            {loading && <TableRow><TableCell colSpan={8}><Stack alignItems="center" sx={{ py: 5 }}><CircularProgress size={28} /><Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>Đang tải biên lai...</Typography></Stack></TableCell></TableRow>}
            {!loading && items.map((item) => <TableRow key={item.id} hover><TableCell><Button size="small" onClick={() => setDetailId(item.id)}>{item.receiptNo}</Button></TableCell><TableCell>{item.payment.tuitionFee.student.code} — {item.payment.tuitionFee.student.fullName}</TableCell><TableCell>{item.payment.tuitionFee.feeNo}</TableCell><TableCell>{item.payment.tuitionFee.items.map((feeItem) => feeItem.classSubject?.subject.name).filter(Boolean).join(", ") || "-"}</TableCell><TableCell>{new Date(item.issuedAt).toLocaleDateString("vi-VN")}</TableCell><TableCell>{statusLabels[item.status]}</TableCell><TableCell align="right">{money(Number(item.amount))}</TableCell><TableCell><Stack direction="row" spacing={0.5}><Button size="small" href={`/api/tuition-receipts/${item.id}/pdf`}>Tải PDF</Button><Button size="small" startIcon={<PrintOutlinedIcon />} href={`/api/tuition-receipts/${item.id}/pdf?inline=1`} target="_blank" rel="noopener noreferrer">In</Button></Stack></TableCell></TableRow>)}
            {!loading && !items.length && <TableRow><TableCell colSpan={8}><Typography sx={{ p: 5 }} textAlign="center" color="text.secondary">Không tìm thấy biên lai phù hợp</Typography></TableCell></TableRow>}
          </TableBody>
        </Table>
