...
export async function POST(request: Request) {
  try { const user = await requireApiUser(); if (user instanceof Response) return user; return apiSuccess(await createBankAccount(bankAccountCreateSchema.parse(await request.json()), user.id), 201); }
