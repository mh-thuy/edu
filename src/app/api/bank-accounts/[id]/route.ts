...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try { const user = await requireApiUser(); if (user instanceof Response) return user; return apiSuccess(await updateBankAccount((await params).id, bankAccountUpdateSchema.parse(await request.json()), user.id)); }
