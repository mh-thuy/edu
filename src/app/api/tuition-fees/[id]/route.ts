...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const user = await requireApiUser(); if (user instanceof Response) return user; const { id } = routeParamsSchema.parse(await params); return apiSuccess(await TuitionService.updateFee(id, tuitionFeeUpdateSchema.parse(await request.json()), user.id)); } catch (error) { return handleApiError(error, "Không thể cập nhật học phí"); } }
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const user = await requireApiUser(); if (user instanceof Response) return user; const { id } = routeParamsSchema.parse(await params); return apiSuccess(await TuitionService.updateStatus(id, tuitionFeeStatusSchema.parse(await request.json()), user.id)); } catch (error) { return handleApiError(error, "Không thể thay đổi trạng thái học phí"); } }
