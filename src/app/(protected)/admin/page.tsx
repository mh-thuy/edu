...
export default function AdminPage() {
  ...
  return <Container maxWidth="xl" sx={{ py: 4 }}>
    <Stack spacing={3}>
      ...
      <Card><CardContent><Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ xs: "stretch", md: "center" }}><Typography fontWeight={700} sx={{ mr: "auto" }}>Bộ lọc doanh thu</Typography><DatePickerField label="Từ ngày" value={dateFrom} onChange={setDateFrom} textFieldProps={{ size: "small" }} /><DatePickerField label="Đến ngày" value={dateTo} onChange={setDateTo} textFieldProps={{ size: "small" }} /><Button variant="contained" onClick={() => void loadStats()}>Áp dụng</Button><Button variant="outlined" onClick={() => { setDateFrom(""); setDateTo(""); }}>Xóa lọc</Button></Stack></CardContent></Card>
